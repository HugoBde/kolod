...
impl Memory {
        ...
        match addr {
            0x00000000..=0x00003fff => {
                return u16::from_le_bytes(
                    self.bios[addr - BIOS_OFFSET..addr - BIOS_OFFSET + 2]
                        .try_into()
                        .unwrap(),
                )
            }
            0x02000000..=0x0203ffff => {
                return u16::from_le_bytes(
                    self.board_wram[addr - BOARD_WRAM_OFFSET..addr - BOARD_WRAM_OFFSET + 2]
                        .try_into()
                        .unwrap(),
                )
            }
            0x03000000..=0x03007fff => {
                return u16::from_le_bytes(
                    self.chip_wram[addr - CHIP_WRAM_OFFSET..addr - CHIP_WRAM_OFFSET + 2]
                        .try_into()
                        .unwrap(),
                )
            }
            0x04000000..=0x040003fe => {
                return u16::from_le_bytes(
                    self.io_reg[addr - IO_REG_OFFSET..addr - IO_REG_OFFSET + 2]
                        .try_into()
                        .unwrap(),
                )
            }
            0x05000000..=0x050003ff => {
                return u16::from_le_bytes(
                    self.pram[addr - PALETTE_RAM_OFFSET..addr - PALETTE_RAM_OFFSET + 2]
                        .try_into()
                        .unwrap(),
                )
            }
            0x06000000..=0x06017fff => {
                return u16::from_le_bytes(
                    self.vram[addr - VRAM_OFFSET..addr - VRAM_OFFSET + 2]
                        .try_into()
                        .unwrap(),
                )
            }
            0x07000000..=0x070003ff => {
                return u16::from_le_bytes(
                    self.oam[addr - OAM_OFFSET..addr - OAM_OFFSET + 2]
                        .try_into()
                        .unwrap(),
                )
            }
            0x08000000..=0x09ffffff => {
                return u16::from_le_bytes(
                    self.game_pak[addr - GAME_PAK_OFFSET..addr - GAME_PAK_OFFSET + 2]
                        .try_into()
                        .unwrap(),
                )
            }
            0x0e000000..=0x0e00ffff => {
                return u16::from_le_bytes(
                    self.game_pak_sram
                        [addr - GAME_PAK_SRAM_OFFSET..addr - GAME_PAK_SRAM_OFFSET + 2]
                        .try_into()
                        .unwrap(),
                )
            }
        ...
        match addr {
            0x00000000..=0x00003fff => {
                return u32::from_le_bytes(
                    self.bios[addr - BIOS_OFFSET..addr - BIOS_OFFSET + 4]
                        .try_into()
                        .unwrap(),
                )
            }
            0x02000000..=0x0203ffff => {
                return u32::from_le_bytes(
                    self.board_wram[addr - BOARD_WRAM_OFFSET..addr - BOARD_WRAM_OFFSET + 4]
                        .try_into()
                        .unwrap(),
                )
            }
            0x03000000..=0x03007fff => {
                return u32::from_le_bytes(
                    self.chip_wram[addr - CHIP_WRAM_OFFSET..addr - CHIP_WRAM_OFFSET + 4]
                        .try_into()
                        .unwrap(),
                )
            }
            0x04000000..=0x040003fe => {
                return u32::from_le_bytes(
                    self.io_reg[addr - IO_REG_OFFSET..addr - IO_REG_OFFSET + 4]
                        .try_into()
                        .unwrap(),
                )
            }
            0x05000000..=0x050003ff => {
                return u32::from_le_bytes(
                    self.pram[addr - PALETTE_RAM_OFFSET..addr - PALETTE_RAM_OFFSET + 4]
                        .try_into()
                        .unwrap(),
                )
            }
            0x06000000..=0x06017fff => {
                return u32::from_le_bytes(
                    self.vram[addr - VRAM_OFFSET..addr - VRAM_OFFSET + 4]
                        .try_into()
                        .unwrap(),
                )
            }
            0x07000000..=0x070003ff => {
                return u32::from_le_bytes(
                    self.oam[addr - OAM_OFFSET..addr - OAM_OFFSET + 4]
                        .try_into()
                        .unwrap(),
                )
            }
            0x08000000..=0x09ffffff => {
                return u32::from_le_bytes(
                    self.game_pak[addr - GAME_PAK_OFFSET..addr - GAME_PAK_OFFSET + 4]
                        .try_into()
                        .unwrap(),
                )
            }
            0x0e000000..=0x0e00ffff => {
                return u32::from_le_bytes(
                    self.game_pak_sram
                        [addr - GAME_PAK_SRAM_OFFSET..addr - GAME_PAK_SRAM_OFFSET + 4]
                        .try_into()
                        .unwrap(),
                )
            }
    ...
    pub fn read_slice(&self, addr: usize, len: usize) -> &[u8] {
        match addr {
            0x00000000..=0x00003fff => {
                return &self.bios[addr - BIOS_OFFSET..addr - BIOS_OFFSET + len]
            }
            0x02000000..=0x0203ffff => {
                return &self.board_wram[addr - BOARD_WRAM_OFFSET..addr - BOARD_WRAM_OFFSET + len]
            }
            0x03000000..=0x03007fff => {
                return &self.chip_wram[addr - CHIP_WRAM_OFFSET..addr - CHIP_WRAM_OFFSET + len]
            }
            0x04000000..=0x040003fe => {
                return &self.io_reg[addr - IO_REG_OFFSET..addr - IO_REG_OFFSET + len]
            }
            0x05000000..=0x050003ff => {
                return &self.pram[addr - PALETTE_RAM_OFFSET..addr - PALETTE_RAM_OFFSET + len]
            }
            0x06000000..=0x06017fff => {
                return &self.vram[addr - VRAM_OFFSET..addr - VRAM_OFFSET + len]
            }
            0x07000000..=0x070003ff => {
                return &self.oam[addr - OAM_OFFSET..addr - OAM_OFFSET + len]
            }
            0x08000000..=0x09ffffff => {
                return &self.game_pak[addr - GAME_PAK_OFFSET..addr - GAME_PAK_OFFSET + len]
            }
            0x0e000000..=0x0e00ffff => {
                return &self.game_pak_sram
                    [addr - GAME_PAK_SRAM_OFFSET..addr - GAME_PAK_SRAM_OFFSET + len]
            }
